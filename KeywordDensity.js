...
function calculateKeywordDensity(text, keyword) {
  ...
  const keyArray = keyword.split(/\s+/);
  let keywordCount = 0;
  for (let i = 0; i < wordArray.length; i++) {
    console.log(wordArray[i]);
    for (let j = 0; j < keyArray.length; j++) {
      if (wordArray[i].toLowerCase() == keyArray[j].toLowerCase()) {
        keywordCount++;
      }
    }
  }
...
async function keywordDensityChecker(url, phrase) {
  ...
  // Check if the phrase exists in the normalized text
  // const found = phraseRegex.test(text);
  // const matches= filteredText.match(phraseRegex);
  // console.log(matches.length);
  // console.log(found);

  // console.log(matchPhrase(text, phrase));

  let freq = matchPhrase(text, phrase);
  // console.log(`Frequency of ${phrase} is ${freq}`);
  let density = ((freq / wordarray.length) * 100).toFixed(2);
  // console.log(density + "%");
  return { frequecy:freq, density}; // Here we are returning the freq and density of the phrase
}
